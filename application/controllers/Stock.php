...
class Stock extends CI_Controller
{
    ...
    {
        $data['row'] = $this->stock_m->get_stock('in')->result();
    ...
    {
        $data['row'] = $this->stock_m->get_stock('out')->result();
    ...
    public function process()
    {
        ...
        $this->stock_m->add_stock($post);
        $this->item_m->update_stock($post);
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('success', "Data Stock-$type berhasil disimpan");
        }
        redirect('stock/'.$type);
